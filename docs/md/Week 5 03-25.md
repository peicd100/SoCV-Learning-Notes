...
不過我也很明確感受到，工具的「介面存在」不代表「每一條路徑都已經可用」。我這次先修掉了一個真的屬於小 bug 的地方：`rand sim` 會在產生 `.sim_main.cpp` 之後去編 CXXRTL testbench，但它原本信任 bundled `yosys-config --datdir` 回傳的 include 路徑；偏偏這台機器實際被 GV 呼叫的是 cache 裡那份 Yosys，而那份 `yosys-config` 又把 datdir 寫成 `/usr/local/share/yosys`，最後就把 `cxxrtl_vcd.h` include 到錯的地方。我後來去改了 `gv/src/ext/sim.cc`，讓它優先從 `GV_YOSYS_BIN_PATH` 附近找真的存在 `backends/cxxrtl/cxxrtl_vcd.h` 的 include root，並在 `g++` 編譯失敗時直接停止，不再硬跑 `./.tb`。

但另一條我真正想用來做 BDD assertion checking 的 prove 路還是沒有完成。我去看了目前工作樹裡的 `gv/src/prove/proveBdd.cpp`，裡面的 `buildPInitialState()`、`buildPTransRelation()`、`buildPImage()`、`runPCheckProperty()` 目前都還是 `TODO`。這件事對我來說反而很有幫助，因為它逼我不要把指令名字誤認成已完成的演算法，也提醒我這次修好的只是 simulation 支線，不是整套 symbolic proof engine。
...
這一段我這次不是只照著打指令，而是真的把卡住的地方修掉再重跑。修完 `gv/src/ext/sim.cc` 之後，`./gv` 本身可以正常啟動，`help` 也能列出 prove commands；我也能成功 `cirread -v ./designs/V3/traffic/traffic.v`，代表 `trafficLight` 這個例子不是只停在文件裡，而是真的可以被目前這份 GV 讀進來。

接著我先跑 `rand sim -sim 5 -rst_n reset -v`，確認 CXXRTL 那條編譯鏈真的被修好。結果這次不再出現 `cxxrtl_vcd.h` 缺失，而是能直接看到 5 個 cycle 的輸出；其中 `time_left` 從 reset 後的 `0`、`0`，進到 `40`、`39`、`38`，而 `p1`、`p2`、`p3` 都維持 0。這至少證明現在 random simulation 這條路是活的，不是只停在 parser 和 command shell。

講義原本後面那個 `rand sim -sim 10 -cont -v`，我也另外試了一下。結果目前這版 GV 不是支援 `-cont`，而是會直接 crash，所以我這次沒有假裝自己把那整段 script 原封不動跑通。取而代之，我改用單次較長的 `rand sim -sim 50 -rst_n reset -output .traffic_sim.txt` 來觀察完整一段狀態演進，這樣比較符合目前工具真實能做到的範圍。

我覺得這個經驗很真實。因為實際做 formal tool 的時候，本來就常常不是「理論會了，工具就一次跑通」，而是要同時面對演算法、介面、依賴、build system。這次我至少把 simulation 支線修到可用，也更清楚地把 prove 支線和 simulation 支線分開了：前者還沒做完，後者現在終於能拿來支撐我這週的觀察。

/// collapse-code
```text title="我修掉 rand sim 小 bug 後，實際重跑的 GV 流程"
$ ./gv
setup> help
...
setup> rand sim -sim 50 -rst_n reset -output .traffic_sim.txt
# 我另外用較長的單次 simulation 看完整一段狀態演進
# cycle 44 ~ 49: p3 = 1，代表進入 YELLOW
# cycle 50: time_left = 60，代表回到 RED 計時

setup> rand sim -sim 1 -cont -v
# 目前這個選項仍然會 crash，不在這次修補範圍內
```
///
...
我這次修完 `rand sim` 之後，先用 `-sim 5` 看短 trace，再用 `-sim 50 -output .traffic_sim.txt` 看比較完整的一段。從 50-cycle 的結果來看，reset 後系統先進入 `GREEN_count = 40` 的倒數，所以 cycle 3 會看到 `time_left = 40`，接著一路遞減到 cycle 43 的 `time_left = 0`。到了 cycle 44 到 49，`p3` 會變成 1，對應 `light == YELLOW`；再到 cycle 50，`time_left` 變成 `60`，代表又切回 `RED_count`。整段 trace 中 `p1` 和 `p2` 都維持 0。

如果只看這次 random simulation，我最直覺的感受當然還是「`p1` 和 `p2` 看起來沒問題」。至少在我真的跑到的這 50 個 cycle 裡，沒有看到非法 state encoding，也沒有看到 counter 超過各自顏色上限。比起之前卡在 header 缺失、連 trace 都看不到，這次我至少終於有一段可以拿來討論的真實波形行為了。

但我還是不敢把這件事直接講成「所以 property 被證明了」。真正關鍵的點在於，`p1` 和 `p2` 的 raw BDD 不一定會是常數 0。BDD 如果只是表示 monitor 這個函數，它看到的是整個 state / input 空間，而不是只有 reset 之後真的能走到的那些 states。以 `p1` 為例，只要 `light = 2'b11`，它就會是 1；可是這個編碼可能在設計的 transition relation 下根本 unreachable。也就是說，monitor 函數本身不為 0，並不代表設計真的會違反 property。
...
我覺得這一段是這週最核心的轉折。前面幾週我比較像是在學怎麼建 BDD、怎麼看 BDD 大小；到這裡，我第一次很明確地看到 BDD 為什麼會變成 verification 的主角。因為它不只是 compact representation，它更是把「所有 reachable states」這種看起來根本存不下的東西，變成可以實際操作的數學物件。這次我把 `rand sim` 修到能跑，讓我比較能從 simulation trace 和 symbolic proof 的落差去理解這件事，而不是只停在抽象概念。
